...
import { getTeamList } from "../../../utils/getTeanList";
import EditBetModal from "../bets/EditBetModal";

const SearchField = ({ editBetCard = false, betsData }) => {
	...
	const initializeTeamsInfo = () => {
		...
		const teamKeys = getTeamList(betsData);
		setTeams(teamKeys);
				...
				<Box my={"5"}>
					...
					<div className="w-full flex flex-col md:flex-row md:justify-between flex-grow gap-8">
						<div className="flex-grow max-w-[700px]">
							...
							<div className="md:min-h-[fit-content] mb-2 md:mb-0 flex flex-col gap-2">
								{betsDataList
									? betsDataList
											.slice(0, 5)
											.map((betData) =>
												!editBetCard ? (
													<BetCard key={betData.id} betData={betData} />
												) : (
													<EditBetModal
														key={betData.id}
														bets={betsData}
